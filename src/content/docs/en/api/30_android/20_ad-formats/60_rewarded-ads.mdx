...
| `{EVENT_TOKEN}` | `sha1( <var>EVENT_ID</var> + <var>Your_Event_Key</var> )` | `e000949f6d851c1f34adae08e6ef1076ba43cf31` | 
| `{EVENT_TOKEN_ALL}` | `sha256( <var>All macros alphabetically, <Abbr>URL</Abbr>-encoded</var> + <var>Your_Event_Key</var> )` <Callout type="note">If `EVENT_TOKEN` is one of your macros, it is omitted from the “All macros alphabetically” string.</Callout> | `eba615583ed59bc679a495ec58439f4b82b5460d822348eff6be5f218702a97a` | 
...
##### Java
```java
AppLovinSdk.getInstance( context ).setUserIdentifier( "<var>user-id</var>" );
```

##### Kotlin
```kotlin
AppLovinSdk.getInstance( context ).userIdentifier = "<var>user-id</var>"
```
...
##### Java
```java
adViewAd.setCustomData( "<var>custom data</var>" ); // banners
adView.setCustomData( "<var>custom data</var>" ); // MRECs
MaxInterstitialAd.showAd(<var>placement</var>, <var>custom data</var>); // interstitials
MaxRewardedAd.showAd(<var>placement</var>, <var>custom data</var>); // rewarded ads
nativeAdLoader.setCustomData( "<var>my custom data</var>" ); // native ads
```
