...
```actionscript-3
var rewardedAdUnitId:String = "<var>ad_unit_ID</var>";
...
| `{EVENT_TOKEN_ALL}` | `sha256( <var>All macros alphabetically, <Abbr>URL</Abbr>-decoded</var> + <var>Your_Event_Key</var> )`<Callout type="note">If `EVENT_TOKEN` is one of your macros, it is omitted from the “All macros alphabetically” string.</Callout> | `eba615583ed59bc679a495ec58439f4b82b5460d822348eff6be5f218702a97a` |
...
##### Banners
```actionscript-3
AppLovinMAX.createBanner(<var>ad-unit-id</var>, AdViewPosition.BOTTOM_CENTER);
AppLovinMAX.setBannerPlacement(<var>ad-unit-id</var>, <var>placement</var>);
```
 
##### <Abbr>MRECs</Abbr>
```actionscript-3
AppLovinMAX.createMRec(<var>ad-unit-id</var>, AdViewPosition.CENTERED);
AppLovinMAX.setMRecPlacement(<var>ad-unit-id</var>, <var>placement</var>);
```

##### Interstitials
```actionscript-3
AppLovinMAX.showInterstitial(<var>ad-unit-id</var>, <var>placement</var>);
```

##### Rewarded Ads
```actionscript-3
AppLovinMAX.showRewardedAd(<var>ad-unit-id</var>, <var>placement</var>);
```
...
```actionscript-3
AppLovinMAX.setUserId("<var>user-id</var>");
```
